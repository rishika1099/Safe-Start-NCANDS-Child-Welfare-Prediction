...
This analysis replaces the simulated-data findings in [`../observations.md`](../observations.md) with results computed on real NYC child-welfare pipeline data. The unit of analysis is aggregate counts by demographic group × fiscal year, so the work shifts from case-level prediction to rate-level disparity, trend, and pipeline analysis.
...
## Methodological note: real vs. simulated

This analysis is materially different from the [simulated-data results](../observations.md):

| Aspect | Simulated track (`results/`) | Real-data track (`results/real_data/`) |
|---|---|---|
| Data | 1,000 fabricated case-level rows | NYC ACS pipeline counts, FY2022–FY2025 (~370,000 SCR intakes pooled) |
| Unit | One row per case | Counts by demographic group × FY |
| Task | Binary classification on a target | Rate, share, and disparity analysis |
| Models | Logistic regression / RF / GBM | Descriptive comparisons, longitudinal trend |
| Output | ROC-AUC / PR-AUC / SHAP | Per-intake rates, disproportionality indices, trend tables |

The simulated track stays in the repo because it's the methodology prototype — group-aware CV, threshold tuning, SHAP — that would be applied to NCANDS Child File case-level data if it were available. The real-data track is a separate analysis on what's publicly downloadable today.

## Caveats

- **Aggregate, not case-level.** No model can be trained at the individual level on this data. Sub-group comparisons are limited to the four axes published (race/ethnicity, gender, language, borough/CD).
- **`^` cells.** A few cells in the source CSV are marked `^` (suppressed counts). They're read as missing; affected subgroups have fewer years available.
- **No demographic denominator.** Per-intake rates compare populations *given* they entered the system. Computing a per-capita "rate of system involvement" would require joining to NYC Census child-population estimates. That denominator join is a natural extension but is not done here.
- **Pipeline definitions evolve.** The CARES program in particular has changed over the period covered, which contributes to the indication-rate trend.
- **No causal claim.** The CARES-diversion explanation is the most parsimonious story given the simultaneous indication-rate drop and CARES-rate rise, but this is a correlational observation in administrative reporting data; a clean causal estimate would need explicit policy-change dates and a DiD design — possible with the SCAN Policies Database (see [docs/NDACAN_NCANDS_Project_Plan.md](../../docs/NDACAN_NCANDS_Project_Plan.md)).

## Reproduce

```bash
python results/real_data/run_real_analysis.py
```
